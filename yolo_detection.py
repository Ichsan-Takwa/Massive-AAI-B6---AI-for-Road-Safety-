...
class project_massive:
    ...
    def load_model(self, model_name):
        model = torch.hub.load('./yolov5', 'custom', path=model_name, source='local', force_reload=True)  # local repo
        return model
    ...
    def get_text(self,labelCnt):
        ...
        for k,v in labelCnt.items():
            if(k== "DangerousDriving"):
                k = "Deinjjers Draiving"
            if(k== "SafeDriving"):
                k = "Sef Draiving"
            if(k== "SleepyDriving"):
                k = "Slipi Draiving"
